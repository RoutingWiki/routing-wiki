...
静态路由可以修改 admin distance 和 metric
...
也可以使用 route-policy 执行 VRF import/export(详见 MPLS VPN 部分)
...
注意：**先加 VRF 再配 IP** 思科会要求先删除所有 IP 才能变更 VRF
